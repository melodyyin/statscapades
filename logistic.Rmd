...
Ok, quite a lot of NAs in this dataset. If we were to get rid of all of the rows, then only `r nrow(train) - max(col_nas)` would remain. However, we can keep all of our observations if we remove the columns with missing values so that only `r sum(col_nas==0)` columns remain. We could also perform imputation using k-nearest neighbors for the variables that are only about 10% missing, but risk adding bias to the training set otherwise.  
